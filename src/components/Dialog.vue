...
export type DialogResolve<T = void> = [T] extends [void]
  ? () => void
  : (value: T) => void;
...
const dialogResolveKey = Symbol("dialogResolve") as InjectionKey<
  DialogResolve<unknown>
>;

const dialogRejectKey = Symbol("dialogReject") as InjectionKey<DialogReject>;
...
export function useDialogContext<T = void>(): {
  ...
  const controller = inject(dialogControllerKey, undefined) as
    | DialogController<T>
    | undefined;
  const resolve = inject(dialogResolveKey, undefined) as
    | DialogResolve<T>
    | undefined;
  const reject = inject(dialogRejectKey, undefined);
