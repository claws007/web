    ...
    return {
        ...
        validator(value, _, options) {
            ...
            if (ret) {
                ...
                return extraRequiredRule?.(value) === false ? msg : ''
            }
        },
    }
}
            ...
            if (value) {
                if (value instanceof Array) {
                    ...
                    if (type === 'string') {
                        ...
                        return +value >= min ? '' : message || `不能小于${min}`
                    }
                }
            }
        },
            ...
            if (value) {
                if (value instanceof Array) {
                    ...
                    if (type === 'string') {
                        ...
                        return +value <= max ? '' : message || `不能大于${max}`
                    }
                }
            }
        },
...
export function range(
    _min: number,
    _max: number,
    options: { type?: TextType; message?: string } = {},
): Rule {
    return $and(min(_min, options), max(_max, options))
}
...
export function alphabetOrNumber(): Rule {
    return {
        validator(value) {
            if (value) {
                return /^[A-Za-z0-9]+$/.test(value) ? '' : '请输入英文或数字'
            }
        },
    }
}
...
// 中文数字字母
export function chineseEnglishNumber(): Rule {
    return {
        validator(value) {
            if (value) {
...
export function integer(): Rule {
    return $and(number(), max(9, { type: 'string' }), {
        validator(value) {
            return !value || !/\./.test(value) ? '' : '请输入整数'
        },
    })
}
...
export function alphabet(): Rule {
    return {
        validator(value) {
            return /^[A-Za-z]+$/.test(value) ? '' : '请使用英文字母'
        },
    }
}
                ...
                if (value) {
                    ...
                    const checkNewIdCard = (code: string) => {
                        ...
                        if (!code) {
                            ...
                            isPass = false
                        } else if (!cityCodes.includes(code.substr(0, 2))) {
                            // 区域数组中不包含需验证的身份证前两位
                            isPass = false
                    ...
                    return oldIdCard.test(value) || checkNewIdCard(value)
                        ? ''
                        : '请输入正确的身份证号码'
                }
            },
...
export const defaultNumberMax = () => max(999999999.9, { type: 'number' })
/**
 * @description 普通数值校验 1 数值 2 保留一位小数
 */
export function fixed1(): Rule {
    return $and(number(), defaultNumberMax(), max(11, { type: 'string' }), {
        validator(value) {
            if (value) {
                return /\.\d\d+$/.test(value) ? '至多一位小数' : ''
            }
        },
    })
}
export const area = () => $and(_fixed2(), positiveNumber())
...
export function _fixed2(): Rule {
    return $and(
        ...
        {
            validator(value) {
                return !!value && /\.\d\d\d+$/.test(value) ? '至多两位小数' : ''
            },
        },
    )
}
export const money = () => $and(_fixed2(), positiveNumber())
export const weight = () => $and(_fixed2(), positiveNumber())
...
export function password(): Rule {
    return $and(required(), range(8, 14), {
        validator(value) {
...
export function taskDuration(_max = 8): Rule {
    return $and(
        fixed1(),
        min(0, { type: 'number', message: '任务持续时长需要大于0小时' }),
        max(_max, { type: 'number', message: `不超过${_max}小时` }),
        range(1, 3),
    )
}
...
export function userName() {
    return $and(chineseEnglishNumber(), range(1, 20))
}
