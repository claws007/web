...
export interface Rule<Entity = any, Value = Entity[keyof Entity]> {
    validator(
        value: Value,
        entity: Entity,
...
export async function validate<T extends Partial<Record<any, any>>>(
    data: T,
    rule: Rules<T>,
    options?: ValidateOptions,
    ...
    return Promise.all(
        Object.entries(rule).map(([k, rs]) => {
            ...
            return Promise.all(
                (rs as Rule[]).map((r) => {
                    return callValidate(r, data[k], data, options).then((d) =>
                        d
                            ? ({
                                  key: k,
                                  value: data[k],
...
export function $and(...rules: Rule[]): Rule {
    const maxs = rules.filter((r) => !!r.max).map((r) => r.max!)
    return {
        required: rules.some((r) => !!r.required),
        max: maxs.length > 0 ? Math.min(...maxs) : undefined,
        validator(value, entity, options) {
            return Promise.all(
                rules.map((r) => r.validator(value, entity, options)),
            ).then((d) => {
                return d.every((d) => !d)
                    ? ''
                    : d
